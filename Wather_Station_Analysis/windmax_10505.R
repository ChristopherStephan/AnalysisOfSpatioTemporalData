...
weather.df <- read.csv("/home/christopher/git/AnalysisOfSpatioTemporalData/data/data/environment/weather-dwd-2667/weather-dwd-10505.csv")
...
wind.df.clust.max.15 <- clust(na.omit(wind.df.clust), u=12, tim.cond = 3/365, clust.max = TRUE, plot = TRUE)
...
mle <- fitgpd(wind.df.clust.max.15[, "obs"], thresh = 12)
par(mfrow = c(2, 2))
