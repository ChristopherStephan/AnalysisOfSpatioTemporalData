...
weather.df <- read.csv("/home/christopher/git/AnalysisOfSpatioTemporalData/data/data/environment/weather-dwd-2667/weather-dwd-10501.csv")
...
prob <- pgpd(25, loc = 25, scale = mle$param["scale"], shape=mle$param["shape"])
prob2rp(prob, npy = npy)

# Return the estimated return period for wind gusts up to 30 m/s
prob <- pgpd(30, loc = 25, scale = mle$param["scale"], shape=mle$param["shape"])
prob2rp(prob, npy = npy)

# Return the estimated return period for wind gusts up to 35 m/s
prob <- pgpd(35, loc = 25, scale = mle$param["scale"], shape=mle$param["shape"])
prob2rp(prob, npy = npy)

# Return the estimated return period for the maximum observed wind gusts 
prob <- pgpd(max(wind.df.clust.max.15[, "obs"]), loc = 25, scale = mle$param["scale"], shape=mle$param["shape"])
prob2rp(prob, npy = npy)